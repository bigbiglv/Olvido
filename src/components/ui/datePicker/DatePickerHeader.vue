...
const isPickerDisabled = (unit: DateUnit, dir: number) =>
  getAdjacentDate(unit, dir).isBefore(props.currentDate, 'day')
...
<template>
  <div class="flex justify-between items-center mb-4 relative z-50">
        ...
        <div class="relative flex justify-center items-center">
          ...
          >
            <Transition name="date-slide">
              <span
                :key="selectedDate.format(picker.format)"
                class="absolute w-full text-center tracking-wide"
                :style="{ '--enter-delay': `${delays[picker.unit]}s` }"
              >
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  :class="
                    isPickerDisabled(picker.unit, -1)
                ...
                >
                  <Transition :name="pickerDirection > 0 ? 'picker-slide-up' : 'picker-slide-down'">
                    <span
                      :key="getAdjacentDate(picker.unit, -1).format(picker.format)"
                      class="absolute w-full text-center text-[12px] font-medium text-muted-foreground/60 transition-colors"
                      :class="!isPickerDisabled(picker.unit, -1) && 'hover:text-foreground'"
                      >{{ getAdjacentDate(picker.unit, -1).format(picker.format) }}</span
                    >
                  </Transition>
                </div>
                ...
                <!-- 当前选中 -->
                <div
                  class="relative flex items-center justify-center h-[30px] w-full bg-primary/10 rounded-lg overflow-hidden"
                >
                  <Transition :name="pickerDirection > 0 ? 'picker-slide-up' : 'picker-slide-down'">
                    <span
                      :key="selectedDate.format(picker.format)"
                      class="absolute w-full text-center text-[14px] font-bold text-primary"
                      >{{ selectedDate.format(picker.format) }}</span
                    >
                  </Transition>
                </div>
                ...
                >
                  <Transition :name="pickerDirection > 0 ? 'picker-slide-up' : 'picker-slide-down'">
                    <span
                      :key="getAdjacentDate(picker.unit, 1).format(picker.format)"
                      class="absolute w-full text-center text-[12px] font-medium text-muted-foreground/60 transition-colors hover:text-foreground"
                      >{{ getAdjacentDate(picker.unit, 1).format(picker.format) }}</span
                    >
                  </Transition>
                </div>
    ...
    <div class="flex items-center gap-2">
      <div
        class="flex items-center text-xs font-medium h-6"
        :class="diffData.diff === 0 ? 'text-primary' : 'text-muted-foreground'"
      >
        <template v-if="diffData.mainType === 'word'">
          <div class="relative h-full flex items-center justify-center overflow-hidden">
            ...
            <Transition name="date-slide">
              <span
                :key="diffData.mainText"
                class="absolute w-full text-center"
                :style="{ '--enter-delay': `${delays.diff}s` }"
                >{{ diffData.mainText }}</span
              >
            </Transition>
          </div>
        </template>
        ...
        <template v-else>
          <div class="relative h-full flex items-center justify-center overflow-hidden">
            ...
            <Transition name="date-slide">
              <span
                :key="diffData.mainNumber"
                class="absolute w-full text-center"
                :style="{ '--enter-delay': `${delays.diff}s` }"
                >{{ diffData.mainNumber }}</span
              >
            </Transition>
          </div>
        ...
        <template v-if="diffData.workdays > 0">
          ...
          <div
            class="relative h-full flex items-center justify-center overflow-hidden text-muted-foreground/80"
          >
            <span class="invisible">{{ diffData.workdays }}</span>
            <Transition name="date-slide">
              <span
                :key="diffData.workdays"
                class="absolute w-full text-center"
                :style="{ '--enter-delay': `${delays.workdays}s` }"
                >{{ diffData.workdays }}</span
              >
            </Transition>
          </div>
      ...
      <Button
        ...
        size="sm"
        class="h-8 text-xs rounded-lg px-4 ml-1"
        @click="emit('confirm')"
        >确认</Button
      >
    </div>
...
.picker-slide-up-leave-to {
  transform: translateY(-15px);
  opacity: 0;
}

.picker-slide-down-enter-from {
  transform: translateY(-15px);
  opacity: 0;
}
.picker-slide-down-leave-to {
  transform: translateY(15px);
  opacity: 0;
}
</style>
