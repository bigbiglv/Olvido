...
const adaptedContentStyle = computed(() => {
  ...
  if (
    hasGsapAnimation.value &&
    (isAnimatingIn.value || isAnimatingFullscreen.value || !dialogInstance.open)
  ) {
    // 动画期间，不强制使用 Vue 的 transform: 'none'，允许 GSAP 控制缩放和位移
    const style = { ...base } as import('vue').CSSProperties
